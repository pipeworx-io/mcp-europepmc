...
`tools/list` at `https://gateway.pipeworx.io/europepmc/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/europepmc_search \
  -H 'Content-Type: application/json' \
  -d '{"query":"CRISPR AND gene editing"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/europepmc_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
